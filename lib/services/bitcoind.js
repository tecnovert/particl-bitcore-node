...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getBlockBalances', this, this.getBlockBalances, 1],
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './particl.conf');

  log.info('Using particl config file:', configPath);
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet/particl.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/particl.conf';
    }
  }
  return networkPath;
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned particl process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean particl process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting particl process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        ...
        log.warn('Restarting particl child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
...
Bitcoin.prototype.getBlockBalances = function(blockhash, callback) {
  var self = this;

  self._tryAllClients(function(client, done) {
    client.getBlockBalances(blockhash, function(err, response) {
      if (err) {
        return done(self._wrapRPCError(err));
      }
      var result = response.result;
